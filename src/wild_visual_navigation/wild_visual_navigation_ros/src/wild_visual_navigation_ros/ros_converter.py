...
def wvn_robot_state_to_torch(robot_state, device="cpu"):
    ...
    # print("vector_state from convertor ---> ", vector_state)
...
def reaction_force_to_torch(reaction_forces, components: list = ["FLx", "FLy", "FLz", "FRx", "FRy", "FRz","RLx", "RLy", "RLz","RRx", "RRy", "RRz"],
                             device="cpu"):
    N = len(components)
    torch_force = torch.zeros(N).to(device)
    i = 0
    # front left leg
    if "FLx" in components:
        torch_force[i] = reaction_forces[0].wrench.force.x
        i += 1
    if "FLy" in components:
        torch_force[i] = reaction_forces[0].wrench.force.y
        i += 1
    if "FLz" in components:
        torch_force[i] = reaction_forces[0].wrench.force.z
        i += 1
    # front right leg
    if "FRx" in components:
        torch_force[i] = reaction_forces[1].wrench.force.x
        i += 1
    if "FRy" in components:
        torch_force[i] = reaction_forces[1].wrench.force.y
        i += 1
    if "FRz" in components:
        torch_force[i] = reaction_forces[1].wrench.force.z
        i += 1
    # rear left leg
    if "RLx" in components:
        torch_force[i] = reaction_forces[2].wrench.force.x
        i += 1
    if "RLy" in components:
        torch_force[i] = reaction_forces[2].wrench.force.y
        i += 1
    if "RLz" in components:
        torch_force[i] = reaction_forces[2].wrench.force.z
        i += 1
    # rear right leg
    if "RRx" in components:
        torch_force[i] = reaction_forces[3].wrench.force.x
        i += 1
    if "RRy" in components:
        torch_force[i] = reaction_forces[3].wrench.force.y
        i += 1
    if "RRz" in components:
        torch_force[i] = reaction_forces[3].wrench.force.z
        i += 1
    return torch_force

def robot_force_state_to_torch(robot_state, components: list = ["FLx", "FLy", "FLz", "FRx", "FRy", "FRz","RLx", "RLy", "RLz","RRx", "RRy", "RRz"],
                                device="cpu"):
    # TODO this should export a SE(3) pose, a R3 twist, a latent, and the labels
    vector_state = [x for x in robot_state.states if x.name == "vector_state"][0]
    vector_vals = vector_state.values
    vector_labels = vector_state.labels

    extracted_vals = [vector_vals[i] for i, label in enumerate(vector_labels) if label in components]
    
    N = len(components)
    torch_force = torch.zeros(N).to(device)

    for i, value in enumerate(extracted_vals):
        torch_force[i] = value
    # torch_force[0] = vals[2]
    # torch_force[1] = vals[5]
    # torch_force[2] = vals[8]
    # torch_force[3] = vals[11]
