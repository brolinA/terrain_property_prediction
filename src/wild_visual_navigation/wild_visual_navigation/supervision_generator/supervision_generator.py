...
class SupervisionGenerator:
    ...
    def update_force_tracking(
        self,
        current_force: torch.tensor,
        desired_force: torch.tensor,
        max_force: float = 1.0
    ):
        """Generates an traversability signal using velocity tracking error

        Args:
            current_force (torch.tensor): Current estimated force from sensor
            desired_force (torch.tensor): Desired force from pinocchio computation
            max_force (float): Max force (magnitude) to scale the error
        ...
        error = (torch.nn.functional.mse_loss(current_force, desired_force)) / max_force
        # Filtering stage
        with torch.no_grad():
            self._state, self._cov = self._kalman_filter_(self._state, self._cov, error)
        error = self._state
